...
![version](https://img.shields.io/badge/version-2.10.0-blue) ![platform](https://img.shields.io/badge/platform-Claude%20Code%20%7C%20Windows%20%7C%20macOS-lightgrey)
...
9. /finalcut         # 【主推】非破坏性精剪：直接用原始片段组装剪映/达芬奇时间线（转场+BGM+字幕+滤镜），最终渲染成片
   /edit             # （可选）无损粗剪预览快速看节奏 + 导出人工精剪交付包；不是成片
10. /publish         # 发布文案+封面 → 半自动上传抖音 →【门禁④ 确认】→ 发布
```
...
- ✅ **精剪成片（`/finalcut`，主推路径，非破坏性）**：直接引用原始镜头组装 NLE 时间线——转场、BGM 对位、
  台词字幕轨、滤镜/调色，变换只在时间线上、最终只渲染一次，**原片零损失**。Resolve 路径确认时间线后可
  **授权自动渲染成片**；剪映路径打开草稿微调后**由用户导出**
- ✅ 粗剪预览（`/edit`，可选）：无损拼接（片段规格一致时零重编码），**仅供快速看节奏**，不是成片；
  同时输出给人工精剪的完整交付包（原始素材 + EDL 剪辑顺序表 + 手工精剪步骤）
- ✅ 发布物料与半自动发布（`/publish`）：候选标题、话题标签、封面图、上传填单，**点击发布前必经确认**
- ✅ 可选剪辑增强（**默认全关**，建项时选或随时让制片人开）：集间交叉衔接（每集开头重放上集结尾几秒）、片头/片尾卡（全剧复用的片名卡/引导卡）
