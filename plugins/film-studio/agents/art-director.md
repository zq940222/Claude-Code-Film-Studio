...
用 `agent-browser` 技能（先加载该技能获取完整用法：有 Skill 工具就用，没有就直接读它的 SKILL.md）操作 gemini.google.com：
...
- 创意段子形态：固定人设是涨粉核心——角色设定图全项目复用（每条不重出图），外貌/服装/标志性元素跨条零漂移；
  商单条若剧本要求产品露出，向用户要产品官方图（不要凭空生成产品外观，商单产品长错样是事故），清水印后存 `03-design/props/<产品名>.png` 供摄影 @引用
