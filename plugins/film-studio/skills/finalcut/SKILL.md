...
- [ ] 时间线**直接引用**原始 `04-footage/ep{NN}/sh*.mp4`，没有用 `/edit` 的粗剪产物（那是二压过的）
- [ ] 字幕按 `subtitle-craft`：竖屏基线在 78%–84% 且**没贴底**、每条 ≤2 行、驻留 1.2–6 秒、
      SRT 用逗号分隔毫秒且 UTF-8、`【独白】`/`【旁白】` 与 `【对白】` 样式能区分
- [ ] 音轨按 `sound-design`：**先拉齐各镜原声电平再铺 BGM**、对白段有 ducking、静音镜已补音、
      整片响度用 ffmpeg 实测过而不是估的
- [ ] 转场按各镜 `transition_in` 决定：同场景内全硬切、转场 ≤0.5 秒、只在时间跳跃/空间大跳/段落分隔处出现
- [ ] `editing` 块的增强项已按配置处理；**开了片头则整条字幕轴已整体偏移**
- [ ] 素材用绝对路径，已提醒用户别移动项目目录
- [ ] 已告知用户三个常见微调点（转场、字幕断句、BGM 音量）和导出方式（Resolve 可授权自动渲染 / 剪映由用户导出）
