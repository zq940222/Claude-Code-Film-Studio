---
name: script
description: 剧本创作阶段。调度编剧 agent 产出大纲、人物小传、分集剧本，迭代修改直到用户确认定稿（门禁①）。用户说"写剧本"、"/script"、"改剧本"时使用。
---

# /script 剧本创作（含门禁①）

## 运行时适配（跨 Agent 兼容）

- **支持 subagent 的运行时**（Claude Code / Hermes Agent 等）：按下文"调度 xxx agent"正常派发子代理执行
- **不支持 subagent 的运行时**（Codex CLI / OpenClaw 等，以插件 bundle 方式安装）：正文提到"调度 X agent"时，
  改为读取插件根 `agents/<X>.md`（本技能目录上两级），以其为工作规范在当前上下文直接执行，效果等同
- **用户确认处**：有 AskUserQuestion 工具就用；没有则直接在对话中提问并等待用户回复，门禁语义不变


## 前置检查

- 确定目标项目（projects/ 下唯一项目则默认它；多个则问）；project.json 必须存在，否则先走 /new-drama

## 流程

1. 收集用户的故事想法（一句话创意即可；用户没想法时让编剧先提 3 个题材内的高概念供选）。
   **顺带问一句本条是否有商单**（品牌推广，任何形态都可能有、创意段子最常见）：有则先把 brief 落
   `01-script/ep{NN}-sponsor.md`（产品/卖点/口播词/禁忌——用户提供什么记什么，缺口播词等关键项要提醒补齐），编剧会按它做植入
2. 调度 **screenwriter** agent：先出 `outline.md` + `characters.md`，给用户过目大纲和人物后再写分集剧本 `ep{NN}.md`
3. 把剧本关键内容（卖点、分集钩子、各集概要）呈现给用户，进入修改迭代循环
4. **门禁①**：用 AskUserQuestion 明确问"剧本是否定稿？"
   - **商单条附加检查**：定稿前确认编剧的商单核对单齐备（卖点覆盖/口播词逐字/禁忌零出现），并提醒用户先把剧本发品牌方过稿再定稿；定稿后让制片人在 project.json 登记 `sponsors.ep{NN}`（品牌名，status=script_ok）
   - 定稿 → 更新 project.json：status.script → approved，提示下一步 `/storyboard`
   - 修改 → 收集意见回到第 2 步

## 注意

- 迭代中的每一稿都直接覆盖文件（git 留痕），不要产生 v1/v2 文件副本
- 人物小传中的外貌描述是后续设定图的源头，确认环节要提醒用户重点看外貌设定
- **短剧靠台词和内心独白叙事**：编剧要用足"台词直给 + 人物内心独白"（扬 AI 之长——脸不可控、音频可控），
  每句台词逐句标 `【对白】/【独白】`（`【旁白】` 仅动漫等形态用），剧本末尾附独白清单；门禁①确认时留意台词是否把身份/动机/心理说清、内心独白是否适度（服务反差与内心戏，不是通篇刷屏）

## 交付自检（门禁① 之前逐条过）

- [ ] `01-script/` 下 `outline.md`、`characters.md`、`ep{NN}.md` 三件齐
- [ ] **每句**台词都带 `【对白】`/`【独白】`（动漫可用 `【旁白】`），没有漏标的裸台词；剧本末尾有独白清单
- [ ] 人物小传写了**外貌**（设定图的唯一源头），不是只有性格与背景
- [ ] 剧本按 `format.medium` 的法则写：短剧黄金 3 秒 / 短片三幕 / 动漫章节感 / 段子黄金 1 秒且梗核翻三番
- [ ] 商单条：`ep{NN}-sponsor.md` 存在，商单核对单三项齐（卖点覆盖 / 口播词逐字 / 禁忌零出现），并已提醒用户送品牌方过稿
- [ ] 门禁① 获用户**明确**确认（不是默认通过）；`status.script` 置 approved；`history/gates.jsonl` 追加了门禁① 的确认行
