...
2. **选整体视频风格（像小云雀那样一次定风格、全剧锁定）**：读插件根的 `templates/style-presets.md`（本技能目录上两级），
   确定 medium 后，用一次 AskUserQuestion 按形态给 3-4 个最合适的预设 + "更多/自定义"（选"更多"就在对话里列全库让用户挑）：
   - 短剧 → `realistic-urban 写实都市` / `cinematic-film 电影质感` / `korean-clean 韩剧清冷` / 更多(港风/古装/3D卡通/自定义…)
   - 电影短片 → `cinematic-film` / `guofeng-ink 国风水墨` / `painterly 油画厚涂` / 更多
   - 动漫 → `anime-2d 2D手绘` / `cartoon-3d 3D卡通` / `painterly` / 更多
